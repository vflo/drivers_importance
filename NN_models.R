    ...
    if (all(n_distinct(faa2$vpd_mean) >= 9,
      ...
      #     bind_rows()->faa2_resample
        ...
        if(test_1 & test_2){
          ...
          type = 1
          model <- sfn_NN( data = faa2, 
                           predictors = c("vpd_mean",'swc','ppfd','pl_species','pl_code'), 
                           nam_target = 'G_sw', weights = NULL,
                           do_predict = TRUE,trainfrac = 0.75, nn = NULL,
                           method="nnet", seed = 1, hidden = NULL )
      }
        ...
        if(test_1 & !test_2){
          ...
          type = 2
          model <- sfn_NN( data = faa2, 
                           predictors = c("vpd_mean",'swc','ppfd','pl_species'), 
                           nam_target = 'G_sw', weights = NULL,
                           do_predict = TRUE,trainfrac = 0.75, nn = NULL,
                           method="nnet", seed = 1, hidden = NULL )
        ...
        if(!test_1 & test_2){
           ...
           type = 3
           model <- sfn_NN( data = faa2, 
                            predictors = c("vpd_mean",'swc','ppfd','pl_code'), 
                            nam_target = 'G_sw', weights = NULL,
                            do_predict = TRUE,trainfrac = 0.75, nn = NULL,
                            method="nnet", seed = 1, hidden = NULL )
             }  
        ...
        if(!test_1 & !test_2){
          ...
          type = 4
          model <- sfn_NN( data = faa2, 
                           predictors = c("vpd_mean",'swc','ppfd'), 
                           nam_target = 'G_sw', weights = NULL,
                           do_predict = TRUE,trainfrac = 0.75, nn = NULL,
                           method="nnet", seed = 1, hidden = NULL )
        ...
        save(model,file=paste0(getwd(),"/data/models/nnet/",unique(x2$si_code),".RData"))
      }else{NULL}
