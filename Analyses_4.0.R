...
# COMPLETE G_log sample MODELS DATA
# path <- "data/models/complete_G_log_sample/"
# site_names <- list.files(path = path)
# models_sample <- purrr::map(site_names,function(x){
#   load(paste0(path,x))
#   print(x)
#   model_env <- model[[2]]
#   model_type <- median(model[[1]]$model_type_complete)
#   faa <- model[[3]]
#   n <- model[[1]]$n_days_complete %>% unique()
# 
#       df <- tibble(n_days_complete = n,
#                r2_G_log = model[[1]]$r2_G_log %>% median(),
#                r2_G_log_vpd = model[[1]]$r2_G_log_vpd %>% median(),
#                r2_G_log_swc = model[[1]]$r2_G_log_swc %>% median(),
#                r2_G_log_ppfd = model[[1]]$r2_G_log_ppfd %>% median(),
#                model_type_complete = model_type,
#                vpd_rel = model[[1]]$vpd_rel %>% median(),
#                swc_rel = model[[1]]$swc_rel %>% median(),
#                ppfd_rel = model[[1]]$ppfd_rel %>% median()) %>%
#     cbind(model_env %>% dplyr::select(-MAT,-MAP)
#           )
#   return(df)
# }) %>% bind_rows()
# save(models_sample,file = "data/complete_G_log_results_sample.RData")
load("data/complete_G_log_results_sample.RData")

# COMPLETE G_log bins MODELS DATA
# path <- "data/models/complete_G_log_bin/"
# site_names <- list.files(path = path)
# models_bin <- purrr::map(site_names,function(x){
#   load(paste0(path,x))
#   print(x)
#   model_env <- model[[1]]
#   model_type <- model[[2]]
...
#   return(df)
# }) %>% bind_rows()
# save(models_bin,file = "data/complete_G_log_results_bin.RData")
load("data/complete_G_log_results_bin.RData")

# path <- "data/models/complete_G_log_GAM/"
# site_names <- list.files(path = path)
# model_gam <- purrr::map(site_names,function(x){
#   load(paste0(path,x))
#   print(x)
#   model_type <- model[[6]]
#   faa <- model[[1]] %>% summary()
...
#     return(df)
#   }) %>% bind_rows()
# model_gam <- model_gam %>% mutate(si_code = str_remove(si_code,".RData"))
# save(model_gam,file = "data/complete_G_log_results_GAM.RData")
load("data/complete_G_log_results_GAM.RData")
...
#Load and prepare Chelsa data biomes to join

biomes <- read.csv("data/biome.csv") #Load modified biomes (Those that we are using)


BIOS <- read_csv("data/CHELSA_BIOS.csv") %>%
  dplyr::select(-lon,-lat,MAT = MAT, MAP = MAP,BIO_4,BIO_15,si_code) %>% 
  mutate(BIO_4 = BIO_4/1000,
         MAT = MAT/10) #Load and prepare BIOS data to join


load("data/LAI_google.RData")
LAI_google <- LAI
colnames(LAI_google) <- c("si_code","lon","lat","LAI_google")
load("data/LAI.RData")
load("data/canopy_height.RData")

## import Data Soil
load("data/clay.RData")
load("data/sand.RData")
load("data/nitrogen.RData")
load("data/ph.RData")
load("data/bedrock_R.RData")
load("data/P_PET.RData")
load("data/P_PETsd.RData")
load("data/PET.RData")
...
data <- models %>%
  ...
  left_join(P_PET %>% dplyr::select(si_code,P_PET),by=c('si_code')) %>%
  left_join(P_PETsd %>% dplyr::select(si_code,P_PETsd),by=c('si_code')) %>%
  left_join(PET %>% dplyr::select(si_code,PET),by=c('si_code')) %>%
  left_join(metadata %>% 
...
pdf(file ="plots/comparative.pdf",width=10,height=6.5)
comparation
dev.off()
...
pdf(file ="plots/dbh_sw.pdf",width=6,height=4)
dbh_sw
dev.off()
...
legend_plot <- ggplot(data_plant%>% 
                        filter(!is.na(si_biome)) %>%
                        rename(BIOME=si_biome),
                      aes(x=r2_G_log_swc,fill = BIOME)) +
  geom_density()+
  theme_bw()+
  scale_fill_manual(values=custom_palette)+
...
mylegend<- gtable::gtable_filter(ggplotGrob(legend_plot), 'guide-box')
...
mylegend$widths <- unit(6, "cm")
mylegend$heights <- unit(6, "cm")
...
tab <- gtable_add_grob(tab,mylegend, t = 115, l = 100)
grid.newpage()
dim(tab)
pdf(file ="plots/r2_G_log_test.pdf",width=6.5,height=6.5)
...
vpd_swc <- vpd_swc[order(factor(vpd_swc$si_biome, levels=levels(data_plant$si_biome))),]
emmeans::test(mod1 %>% emmeans::emmeans('si_biome'),0) -> vpd_swc_p
vpd_swc_p <- vpd_swc_p[order(factor(vpd_swc_p$si_biome, levels=levels(data_plant$si_biome))),]

mod1 <- lm((r2_G_log_vpd-r2_G_log_ppfd)~si_biome , data = data_plant, weights = n_days_complete)
summary(mod1)
multcomp::cld(mod1 %>% emmeans::emmeans('si_biome'), 
              Letters = LETTERS) -> vpd_ppfd
vpd_ppfd <- vpd_ppfd[order(factor(vpd_ppfd$si_biome, levels=levels(data_plant$si_biome))),]
emmeans::test(mod1 %>% emmeans::emmeans('si_biome'),0) -> vpd_ppfd_p
vpd_ppfd_p <- vpd_ppfd_p[order(factor(vpd_ppfd_p$si_biome, levels=levels(data_plant$si_biome))),]

mod1 <- lm((r2_G_log_swc-r2_G_log_ppfd)~si_biome , data = data_plant, weights = n_days_complete)
summary(mod1)
multcomp::cld(mod1 %>% emmeans::emmeans('si_biome'), 
              Letters = LETTERS) -> swc_ppfd
swc_ppfd <- swc_ppfd[order(factor(swc_ppfd$si_biome, levels=levels(data_plant$si_biome))),]
emmeans::test(mod1 %>% emmeans::emmeans('si_biome'),0) -> swc_ppfd_p
swc_ppfd_p <- swc_ppfd_p[order(factor(swc_ppfd_p$si_biome, levels=levels(data_plant$si_biome))),]
...
swc_alone <- swc_alone[order(factor(swc_alone$si_biome, levels=levels(data_plant$si_biome))),]
...
vpd_alone <- vpd_alone[order(factor(vpd_alone$si_biome, levels=levels(data_plant$si_biome))),]
emmeans::test(mod1 %>% emmeans::emmeans('si_biome'),0)
...
ppfd_alone <- ppfd_alone[order(factor(ppfd_alone$si_biome, levels=levels(data_plant$si_biome))),]
emmeans::test(mod1 %>% emmeans::emmeans('si_biome'),0)
# plot(mod1)
...
pca_out <- FactoMineR::PCA(fee[,-1], graph=TRUE)
# reduced_Data = fee[,hc]
# # print (reduced_Data)
...
performance::check_distribution(mod2)
performance::check_heteroscedasticity(mod2)
anova(mod2)
...
performance::check_distribution(mod3)
...
performance::check_heteroscedasticity(mod4)
anova(mod4)
...
performance::check_distribution(mod5)
performance::check_heteroscedasticity(mod5)
...
##table bioclim models relative

m5 <- summary(mod5)$coefficients
m5 <- m5 %>% 
  as_tibble(rownames = "Variable") %>% 
  dplyr::select(Variable,Estimate,p_val=`Pr(>|t|)`) %>% 
  mutate(esti = paste(round(Estimate,3),
...
gg2_G_log_vpd$si_biome <- factor(gg2_G_log_vpd$si_biome, 
                                 levels = c("DRY", 
                                            "WOOD", 
                                            "TEMP",
                                            "BOR",
...
pdf(file ="plots/spa_plots.pdf",width=12,height=6.5)
spa_plots2
dev.off()
...
MAP <- raster("data/rasters/MAP_resample.tif")
MAT <- raster("data/rasters/MAT_resample.tif")
MAT <- MAT/10
BIO_4 <- raster("data/rasters/BIO_4_resample.tif")
BIO_15 <- raster("data/rasters/BIO_15_resample.tif")
BIO_4 <- BIO_4/1000
# LAI <- raster("data/rasters/LAI_resample.tif")
LAI <- raster("data/rasters/LAI_google_resample.tif")
st_height <- raster("data/rasters/st_height_resample.tif")
clay <- raster("data/rasters/clay_resample.tif")
sand <- raster("data/rasters/sand_resample.tif")
bedrock <- raster("data/rasters/bedrock_R_resample.tif")
# bedrock_R <- raster("data/rasters/bedrock_R_resample.tif")
nitrogen <- raster("data/rasters/nitrogen_resample.tif")
ph <- raster("data/rasters/ph_resample.tif")
# MO <- raster("data/rasters/MO_resample.tif")
# PPET <- raster("data/rasters/PPET_resample.tif")
P_PET <- raster("data/rasters/P_PET_resample.tif")
P_PETsd <- raster("data/rasters/P_PETsd_resample.tif")
PET <- raster("data/rasters/PET_resample.tif")
# TOTN <- raster("data/rasters/TOTN_resample.tif")
# si_elev<-raster("data/rasters/elevation_resample.tif")
PPET <- MAP/PET
# P_PET <- P-PET
# bedrock_R_cat <- bedrock_R
# bedrock_R[bedrock_R>=200] <- "deep"
# bedrock_R[bedrock_R<200&bedrock>50]<-"medium"
# bedrock_R[bedrock_R<=50] <- "shallow"
...
# pdf(file ="plots/G_log_maps.pdf",width=13, height=9.5, compress = FALSE)
# gg_maps
# dev.off()

ggsave(
  "plots/G_log_maps.png",
  gg_maps,
  width = 27,
  height = 20,
  dpi = 600,
...
# pdf(file ="plots/triplot_map.pdf",width=10,height=5)
# triplot_map
# dev.off()

ggsave(
  "plots/triplot_map.png",
  triplot_map,
  width = 30,
  height = 15,
  dpi = 600,
