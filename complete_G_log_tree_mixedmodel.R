...
furrr::future_map(site_names,.progress=TRUE, .f=function(.x){
  ...
  if (!is.null(faa) & any(!is.na(faa$swc))){
    ...
    if (all(n_distinct(faa2$vpd_mean) >= 9,
      ...
      test_1 <- unique(faa2$pl_code) %>% length()>1
      
      if(test_1){
        
        type = 1
      
      model_G_sw_log <- try(lmer(G_sw~log_vpd_mean+log_swc+log_ppfd + (1|pl_code),
                                 data = faa2))
      model_G_sw_log_vpd <- try(lmer(G_sw~log_vpd_mean+ (1|pl_code),data = faa2))
      model_G_sw_log_swc <- try(lmer(G_sw~log_swc+ (1|pl_code),data = faa2))
      model_G_sw_log_ppfd <- try(lmer(G_sw~log_ppfd+ (1|pl_code),data = faa2))
      ...
      df <- tibble(n_days_complete = nrow(faa2),
        ...
        save(model,file=paste0(getwd(),
                               "/data/models/complete_G_log_tree_mixedmodel/",
