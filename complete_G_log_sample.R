...
set.seed(7)
      ...
      purrr::map(c(1:100), function(x){
        ...
        #       res <- sample_n(y, 50)
        #     }else{
        #       res <- sample_n(y, len/2)
        #     }
        #     if(len<=20 & len>10){res <- sample_n(y, 10)}
        #     if(len<10){res <- y}
        #     return(res)
        #   }) %>% 
        ...
        if(test_1 & test_2){
          ...
          type = 1
            ...
            model_G_sw_log_vpd <- try(lmer(G_sw~log_vpd_mean+
                                             ...
                                             (1|pl_species:pl_code),REML = TRUE,data = faa2_resample))
        ...
        if(!test_1 & !test_2){
          ...
          model_G_sw_log <- try(lm(G_sw~log_vpd_mean+log_swc+log_ppfd,data = faa2_resample))
          model_G_sw_log_vpd <- try(lm(G_sw~log_vpd_mean,data = faa2_resample))
          model_G_sw_log_swc <- try(lm(G_sw~log_swc,data = faa2_resample))
          model_G_sw_log_ppfd <- try(lm(G_sw~log_ppfd,data = faa2_resample))
        ...
        save(model,file=paste0(getwd(),"/data/models/complete_G_log_sample/",unique(x2$si_code),".RData"))
      }else{NULL}
