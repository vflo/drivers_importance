...
path <- "~/Chapter_2_mod/data/site_daylight/"
site_names <- list.files(path = path)

# source('~/Chapter_2_mod/Individual_plant_analyses/treatment_list_filter.R')
source('~/Chapter_3_mod/treatment_filter_only_control.R')

load("~/Chapter_2_mod/data/swc_ERA5_land.RData") #nearest cell extract ERA5 land 9x9km 1980 to present
...
## import PET
PET <- read_csv('~/Chapter_2_mod/data/PET.csv') %>% 
  dplyr::rename(si_lat = lati,si_long = long)
BIOS <- read_csv("~/Chapter_2_mod/data/CHELSA_BIOS.csv") %>% mutate(si_long = lon, si_lat = lat)

load("~/Chapter_2_mod/data/sand.RData")
load("~/Chapter_2_mod/data/clay.RData")
load("~/Chapter_2_mod/data/elevation.RData")

load("~/Chapter_2_mod/data/sw_ERA5_18.RData")
load("~/Chapter_2_mod/data/sw_ERA5_6.RData")
