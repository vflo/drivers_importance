...
done_files <- list.files("data/models/GAM")
site_names <- site_names[!site_names %in% done_files]
...
furrr::future_map(site_names,.progress=TRUE,.f=function(.x){
  ...
  if (!is.null(faa) & any(!is.na(faa$swc))){
    ...
    if (all(n_distinct(faa2$vpd_mean) >= 9,
      ...
      map(c(1:100), function(x){
        
        faa2 %>% 
          split(.[['pl_code']],drop = FALSE) %>%
          map(function(x){
            nrow(x)
            res <- sample_n(x,nrow(x)/2)
            return(res)
          }) %>% 
          bind_rows()->faa2_resample
        ...
        if(test_1 & test_2){
          ...
          model_vpd_swc <- try(mgcv::bam(G_sw~s(vpd_mean,by=pl_species,k=k_min_compl)+
                                   s(swc,by=pl_species,k=k_min_compl)+
                                   s(vpd_mean,pl_code,by=pl_species,k=k_min_compl, bs="fs")+
                                   s(swc,pl_code,by=pl_species,k=k_min_compl, bs="fs"),
                                 data = faa2_resample%>%
          ...
          model_vpd_ppfd <- try(mgcv::bam(G_sw~s(vpd_mean,by=pl_species,k=k_min_compl)+
                                   s(ppfd,by=pl_species,k=k_min_compl)+
                                   s(vpd_mean,pl_code,by=pl_species,k=k_min_compl, bs="fs")+
                                   s(ppfd,pl_code,by=pl_species,k=k_min_compl, bs="fs"),
                                 data = faa2_resample%>%
          ...
          model_swc_ppfd <- try(mgcv::bam(G_sw~s(swc,by=pl_species,k=k_min_compl)+
                                   s(ppfd,by=pl_species,k=k_min_compl)+
                                   s(swc,pl_code,by=pl_species,k=k_min_compl, bs="fs")+
                                   s(ppfd,pl_code,by=pl_species,k=k_min_compl, bs="fs"),
                                 data = faa2_resample%>%
          ...
          model_swc <- try(mgcv::bam(G_sw~s(swc,by=pl_species,k=k_min)+
                                       s(swc,pl_code,by=pl_species,k=k_min, bs="fs"),
                                     data = faa2_resample%>%
                                       mutate(pl_code = as.factor(pl_code),
                                              pl_species = as.factor(pl_species)),
                                     method = "fREML"),silent = TRUE) #check error
        ...
        if(test_1 & !test_2){
          ...
          model <- mgcv::bam(G_sw~
                               ...
                               s(swc,pl_species,k=k_min_compl, bs="fs")+
          ...
          model_swc <- mgcv::bam(G_sw~
                                   s(swc,k=k_min)+
                                   s(swc,pl_species,k=k_min, bs="fs"),
                                 data = faa2_resample%>%
                                   mutate(pl_code = as.factor(pl_code),
                                          pl_species = as.factor(pl_species)),
                                 method = "fREML")
        ...
        if(!test_1 & test_2){
          ...
          model <- try(mgcv::bam(G_sw~s(vpd_mean,k=k_min_compl)+
                                   ...
                                   s(swc,pl_code,k=k_min_compl, bs="fs")+
          ...
          model_vpd_swc <- try(mgcv::bam(G_sw~s(vpd_mean,k=k_min_compl)+
                                   s(swc,k=k_min_compl)+
                                   s(vpd_mean,pl_code,k=k_min_compl, bs="fs")+
                                   s(swc,pl_code,k=k_min_compl, bs="fs"),
                                 data = faa2_resample%>%
          ...
          model_vpd_ppfd <- try(mgcv::bam(G_sw~s(vpd_mean,k=k_min_compl)+
                                   s(ppfd,k=k_min_compl)+
                                   s(vpd_mean,pl_code,k=k_min_compl, bs="fs")+
                                   s(ppfd,pl_code,k=k_min_compl, bs="fs"),
                                 data = faa2_resample%>%
          ...
          model_swc_ppfd <- try(mgcv::bam(G_sw~s(swc,k=k_min_compl)+
                                   s(ppfd,k=k_min_compl)+
                                   s(swc,pl_code,k=k_min_compl, bs="fs")+
                                   s(ppfd,pl_code,k=k_min_compl, bs="fs"),
                                 data = faa2_resample%>%
          ...
          model_swc <- try(mgcv::bam(G_sw~s(swc,k=k_min)+
                                       s(swc,pl_code,k=k_min, bs="fs"),
                                     data = faa2_resample%>%
                                       mutate(pl_code = as.factor(pl_code),
                                              pl_species = as.factor(pl_species)),
                                     method = "fREML"),silent = TRUE) #check error
        ...
        if(!test_1 & !test_2){
          ...
          model <- mgcv::gam(G_sw~s(vpd_mean,k=k_min_compl)+s(swc,k=k_min_compl)+s(ppfd,k=k_min_compl), 
                             data = faa2_resample)
          
          model_vpd_swc <- mgcv::gam(G_sw~s(vpd_mean,k=k_min_compl)+s(swc,k=k_min_compl), 
                             data = faa2_resample)
          
          model_vpd_ppfd <- mgcv::gam(G_sw~s(vpd_mean,k=k_min_compl)+s(ppfd,k=k_min_compl), 
                             data = faa2_resample)
          
          model_swc_ppfd <- mgcv::gam(G_sw~s(swc,k=k_min_compl)+s(ppfd,k=k_min_compl), 
                             data = faa2_resample)
          
          model_vpd <- mgcv::gam(G_sw~s(vpd_mean,k=k_min), data = faa2_resample)
          
          model_swc <- mgcv::gam(G_sw~s(swc,k=k_min), data = faa2_resample)
          
          model_ppfd <- mgcv::gam(G_sw~s(ppfd,k=k_min), data = faa2_resample)
          ...
          df <- tibble(trial = x,
                       n_days_complete = nrow(faa2_resample),
                       r2_model = model %>% summary() %>% .[['r.sq']],
                       r2_model_vpd_swc = model_vpd_swc %>% summary() %>% .[['r.sq']],
                       r2_model_vpd_ppfd = model_vpd_ppfd %>% summary() %>% .[['r.sq']],
                       r2_model_swc_ppfd = model_swc_ppfd %>% summary() %>% .[['r.sq']],
                       r2_model_vpd = model_vpd %>% summary() %>% .[['r.sq']],
                       r2_model_ppfd = model_ppfd %>% summary() %>% .[['r.sq']],
                       r2_model_swc = model_swc %>% summary() %>% .[['r.sq']],
        ...
        save(df_res,file=paste0(getwd(),"/data/models/GAM/",unique(x2$si_code),".RData"))
      }else{NULL}
